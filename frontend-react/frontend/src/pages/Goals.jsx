...
function Goals() {
  ...
  const { form: simulationForm, result: simulationResult } = getLastSimulation();

  const purchaseItem =
    simulationResult?.itemName || simulationForm?.itemName || "this purchase";

  useEffect(() => {
    const { result: latestSimulationResult } = getLastSimulation();

    if (latestSimulationResult?.goalDelayMonths !== undefined) {
      setResult((prev) => ({
        ...prev,
        delayMonths: latestSimulationResult.goalDelayMonths,
      }));
    }
  }, []);
  ...
  const calculateDelay = async (e) => {
    ...
    try {
      const res = await apiRequest("/goals/delay", "POST", payload);
      setResult(res.data);
    } catch (err) {
      console.error("Goal delay API error:", err.message);
      alert(err.message || "Goal delay calculation failed");
    }
  };

  const gap = Math.max(
    0,
    Number(formData.goalAmount || 0) - Number(formData.currentSavings || 0)
  );

  const withoutWidth = Math.min(100, Math.max(8, Number(result.withoutPurchaseMonths || 0) * 3));
  const afterWidth = Math.min(100, Math.max(8, Number(result.afterPurchaseMonths || 0) * 3));
    ...
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
      <div className="mb-8">
        ...
        <p className="text-sm text-slate-500 mt-0.5">
          See how {purchaseItem} affects tomorrow&apos;s goals.
        </p>
      </div>
      ...
      <div className="grid gap-5 grid-cols-1 lg:grid-cols-[0.9fr_1.5fr]">
        <form onSubmit={calculateDelay} className={cardCls}>
          ...
          {[
            ["Goal Amount (₹)", "goalAmount", "150000"],
            ["Current Savings (₹)", "currentSavings", "80000"],
            ["Monthly Savings (₹)", "monthlySavings", "5000"],
            [`Purchase Cost (₹) - ${purchaseItem}`, "purchaseCost", "90000"],
        ...
        <div className={cardCls}>
          ...
          <div className="rounded-lg border border-white/[0.07] bg-white/[0.03] p-4 mb-3">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-slate-400">Without {purchaseItem}</span>
          ...
          <div className="rounded-lg border border-white/[0.07] bg-white/[0.03] p-4 mb-5">
            <div className="flex justify-between text-sm mb-2">
              <span className="text-slate-400">After buying {purchaseItem}</span>
          ...
          <div className="rounded-lg border border-rose-500/20 bg-rose-500/[0.07] p-5 flex items-center gap-4 mb-5">
            ...
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-rose-400 mb-0.5">
                Goal Delay
              </p>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Your goal gets pushed back if you buy {purchaseItem} today.
              </p>
            </div>
          </div>
          ...
          <div className="grid gap-3 grid-cols-1 sm:grid-cols-3">
            {[
              {
                label: "Current Gap",
                val: `₹${gap.toLocaleString("en-IN")}`,
                cls: "text-indigo-300",
              },
              {
                label: `${purchaseItem} Impact`,
                val: `₹${Number(formData.purchaseCost || 0).toLocaleString("en-IN")}`,
                cls: "text-amber-300",
              },
              {
                label: "Monthly Saving",
                val: `₹${Number(formData.monthlySavings || 0).toLocaleString("en-IN")}`,
