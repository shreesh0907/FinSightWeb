...
const kpis = [
  { icon: BarChart2,   label: "Health Score",    value: `${dashboardData.healthScore}/100`,                                  delta: "−14 this month",    up: false },
  { icon: Wallet,      label: "Current Savings",  value: `₹${dashboardData.currentSavings.toLocaleString("en-IN")}`,         delta: "+₹25,000 this month", up: true  },
  { icon: Target,      label: "Active Goals",     value: `${dashboardData.activeGoals} goals`,                               delta: "In progress",        up: true  },
  { icon: AlertCircle, label: "Risk Level",       value: dashboardData.riskLevel,                                            delta: "Stable since May",   up: null  },
];
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 space-y-5">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-0.5">Your complete financial picture at a glance.</p>
      </div>
      ...
      {/* ── Row 2: Savings Trend + Spending Breakdown ── */}
        ...
        <Card>
          ...
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={savingsTrend} margin={{ left: -8 }}>
              ...
              <Area type="monotone" dataKey="savings" stroke="#6366f1" strokeWidth={2}
                    fill="url(#sg)" dot={{ r: 3.5, fill: "#6366f1", strokeWidth: 0 }} />
            </AreaChart>
          </ResponsiveContainer>
        </Card>
      ...
      {/* ── Row 3: Health Score + Allowance vs Expense ── */}
      ...
      {/* ── Row 4: Goal Progress + AI Advice ── */}
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-[1.4fr_1fr]">
        <Card>
          <CardTitle icon={Target} title="Goal Progress" />
          <div className="space-y-5 mb-6">
            {goalProgress.map(({ name, target, saved }) => {
          ...
          <ResponsiveContainer width="100%" height={130}>
            <BarChart layout="vertical" data={goalProgress} margin={{ left: 0, right: 8 }}>
              <XAxis type="number" hide />
              <YAxis type="category" dataKey="name" width={105} tick={{ fill: "#475569", fontSize: 11 }}
                     {...axisProps} />
              <Tooltip formatter={(v) => `₹${v.toLocaleString("en-IN")}`}
                contentStyle={{ background: "#13131f", border: "1px solid rgba(255,255,255,0.08)",
        ...
        <Card>
          ...
          <div className="rounded-lg border border-white/[0.07] bg-white/[0.03] p-4 mb-4">
            <p className="text-xs font-semibold text-emerald-400 uppercase tracking-wide mb-1">Latest advice</p>
